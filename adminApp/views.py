    ...
    if request.method == "POST":
        ...
        try:
            User.objects.create_user(
                username=username, email=email, password=password_generator()
            )
            doctor = DoctorInfo(
...
def view_specialty(request):
    specialty_list = (
        DoctorSpecialization.objects.values("specialization_id__name")
        .annotate(speciality_count=Count("specialization_id"))
        .order_by("specialization_id__name")
    )
            ...
            try:
                if fields[3] == "Gender":
                    ...
                    office = Office(
                        ...
                        country=country,
                        zipcode=zipcode,
                        time_slot_per_patient_time=time_slot,
                        first_consultation_fee=fee,
